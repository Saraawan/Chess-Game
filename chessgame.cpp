...
struct ChessBoard{
    ...
    map<Piece, int> white_captures;
map<Piece, int> black_captures;
    ...
    bool makeMove(Pos from, Pos to){
        ...
        flipTurn();
        return true;
         if (opponentPieces().count(to)) {
        Piece capturedPiece = opponentPieces()[to];
        if (turn == Turn::white) {
            white_captures[capturedPiece]++;
        } else {
            black_captures[capturedPiece]++;
        }
        opponentPieces().erase(to);
    }
    ...
    void printBoard(){
          ...
          static map<Piece, char> sprites = { {Piece::white_pawn,'P'}, {Piece::black_pawn,'p'}, {Piece::rook,'R'}, {Piece::knight,'N'},
    ...
    void printHelp(){
        cout<<endl<<" * h: help, \n * q: quit, \n * p: show board, \n * c: show coordinates inside squares"<<endl<<" \n * Input format: yxyx is from-to coordinates,\n e.g: 1715 moves (x,y)=(1,7) to (x,y)=(1,5)"<<endl<<endl;
    }
